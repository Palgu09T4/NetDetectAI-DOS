...
model = joblib.load('dos_rf_model.joblib')
scaler = joblib.load('scaler.joblib')

# Feature columns expected by the model (make sure these exactly match your trained model's features)
feature_columns = [
...
def predict_dos_from_csv(csv_path):
    ...
    # Drop those columns before prediction
    ...
    # Convert to numeric and handle NaNs/infs
    ...
    # Combine results with selected original columns
...
def predict_dos_from_input(input_row):
    # input_row: list or array of feature values in the exact order of feature_columns
